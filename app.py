...
def get_db():
    return mysql.connector.connect(
        host=os.environ.get("DB_HOST", "dermacare-2026-hengkoengromdoul-0202.h.aivencloud.com"),
        port=int(os.environ.get("DB_PORT", "17769")),
        user=os.environ.get("DB_USER", "avnadmin"),
        password=os.environ.get("DB_PASSWORD", "AVNS_SskU_3SoAX47o91WPMH"),
...
def patient_dashboard():
    ...
    if pat:
        pid = pat["patient_id"]
        # one row per appointment, with both payment totals
        cursor.execute("""
            SELECT a.appointment_id, a.appointment_date_time, a.status, a.booking_fee,
                   d.name AS doctor_name, d.specialization,
                   (SELECT i.invoice_id FROM invoice i
                      WHERE i.appointment_id = a.appointment_id AND i.invoice_type='consultation' LIMIT 1) AS consult_invoice_id,
                   (SELECT pay.status FROM invoice i
                      JOIN payment pay ON pay.invoice_id = i.invoice_id
                      WHERE i.appointment_id = a.appointment_id AND i.invoice_type='consultation' LIMIT 1) AS consult_payment_status,
                   (SELECT pay.payment_method FROM invoice i
                      JOIN payment pay ON pay.invoice_id = i.invoice_id
                      WHERE i.appointment_id = a.appointment_id AND i.invoice_type='consultation' LIMIT 1) AS consult_payment_method,
                   (SELECT i.total_amount FROM invoice i
                      WHERE i.appointment_id = a.appointment_id AND i.invoice_type='medicine' LIMIT 1) AS pharmacy_total,
                   (SELECT pay.status FROM invoice i
                      JOIN payment pay ON pay.invoice_id = i.invoice_id
                      WHERE i.appointment_id = a.appointment_id AND i.invoice_type='medicine' LIMIT 1) AS pharmacy_payment_status,
                   (SELECT pay.payment_method FROM invoice i
                      JOIN payment pay ON pay.invoice_id = i.invoice_id
                      WHERE i.appointment_id = a.appointment_id AND i.invoice_type='medicine' LIMIT 1) AS pharmacy_payment_method
            FROM appointment a
        ...
        cursor.execute("""
            SELECT pr.prescription_id, pr.prescription_date_time,
                   pr.diagnosis, pr.notes, pr.status,
                   d.name AS doctor_name,
                   GROUP_CONCAT(CONCAT(m.medicine_name, ' (x', pm.quantity, ')') SEPARATOR ', ') AS medicines_list,
                   COUNT(pm.prescription_medicine_id) AS medicine_count,
                   COALESCE(SUM(m.price * pm.quantity), 0) AS total_price
...
def pharmacist_dashboard():
    ...
    cursor.execute("""
        SELECT pr.prescription_id, pr.diagnosis, pr.notes, pr.status,
               pr.prescription_date_time, pr.dispensed_at, pr.appointment_id,
               d.name AS doctor_name,
...
def update_medicine(medicine_id):
    ...
    try:
        cursor.execute("""
            UPDATE medicine
            SET medicine_name = %s, medicine_type = %s,
                price = %s, stock_quantity = %s, expiry_date = %s
            WHERE medicine_id = %s
        """, (name, mtype, price, stock, expiry_date, medicine_id))
        db.commit()
        log_action(session["user_id"], "UPDATE_MEDICINE", f"Updated medicine ID {medicine_id}")
        flash("Medicine updated successfully!", "success")
    except Exception as e:
        db.rollback()
        print(f"UPDATE MEDICINE ERROR: {e}")
        flash("Failed to update medicine.")
    finally:
        cursor.close()
        db.close()
    return redirect(url_for("pharmacist_dashboard"))
...
def delete_medicine(medicine_id):
    ...
    cursor = db.cursor()
    try:
        # safety: don't delete medicines that are in active prescriptions
        cursor.execute("""
            SELECT COUNT(*) FROM prescription_medicine WHERE medicine_id = %s
        """, (medicine_id,))
        in_use_count = cursor.fetchone()[0]
        if in_use_count > 0:
            flash(f"Cannot delete: this medicine is used in {in_use_count} prescription(s). Update stock to 0 instead.", "warning")
        else:
            cursor.execute("DELETE FROM medicine WHERE medicine_id = %s", (medicine_id,))
            db.commit()
            log_action(session["user_id"], "DELETE_MEDICINE", f"Deleted medicine ID {medicine_id}")
            flash("Medicine deleted successfully!", "success")
    except Exception as e:
        db.rollback()
        print(f"DELETE MEDICINE ERROR: {e}")
        flash("Failed to delete medicine.")
    finally:
        cursor.close()
        db.close()
    return redirect(url_for("pharmacist_dashboard"))
